...
static void display_cabdiag_status(const struct phy_cabdiag_req *status)
{
	...
	if (status->pairs_bitmask & CABDIAG_PAIR_A_MASK)
		if (status->pairs[CABDIAG_PAIR_A].length != CD_NOT_SUPPORT)
			printf("    Pair A: %s, Loop Length: %d m\n",
	...
	if (status->pairs_bitmask & CABDIAG_PAIR_B_MASK)
		if (status->pairs[CABDIAG_PAIR_B].length != CD_NOT_SUPPORT)
			printf("    Pair B: %s, Loop Length: %d m\n",
	...
	if (status->pairs_bitmask & CABDIAG_PAIR_C_MASK)
		if (status->pairs[CABDIAG_PAIR_C].length != CD_NOT_SUPPORT)
			printf("    Pair C: %s, Loop Length: %d m\n",
	...
	if (status->pairs_bitmask & CABDIAG_PAIR_D_MASK)
		if (status->pairs[CABDIAG_PAIR_D].length != CD_NOT_SUPPORT)
			printf("    Pair D: %s, Loop Length: %d m\n",
...
static void display_error_msg(enum phy_cabdiag_op_sta op_status)
{
	switch(op_status) {
	case CD_REQ_INVALID_PAIR_MASK:
		printf("Error : Invalid pair mask configuration\n");
		break;
	case CD_REQ_INVALID_TIMEOUT:
		printf("Error : Invalid timeout configuration\n");
		break;
	case CD_REQ_REJECTED_BUSY:
		printf("Error : Request rejected due to busy\n");
		break;
	case CD_STATUS_FAILED_TIMEOUT:
		printf("Error : Failed due to Timeout\n");
		break;
	case CD_STATUS_FAILED_INVALID:
		printf("Error : Failed due to Invalid\n");
		break;
	}
	...
	mnl_attr_for_each(attr, nlhdr, GENL_HDRLEN) {
		...
		if (type == CABDIAG_OP_ATTR_REQUEST) {
			mnl_attr_for_each_nested(nestattr, attr) {
				...
				switch (nesttype) {
				case CABDIAG_REQ_ATTR_OP_STA:
					op_status = mnl_attr_get_u8(nestattr);
					if (op_status != CD_STATUS_SUCCESS) {
						display_error_msg(op_status);
						exit(EXIT_FAILURE);
					} else {
...
int main(int argc, char *argv[])
{
	...
	if (argc == 3 && !strcmp(argv[2], "request")) {
		...
		mnl_attr_put_u8(nlh, CABDIAG_REQ_ATTR_PAIRS_MASK,
				(CABDIAG_PAIR_A_MASK | CABDIAG_PAIR_B_MASK |
				 CABDIAG_PAIR_C_MASK| CABDIAG_PAIR_D_MASK));

		mnl_attr_put_u8(nlh, CABDIAG_REQ_ATTR_TIMEOUT, 254);
